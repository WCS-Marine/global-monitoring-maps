...
total_ng_overlap <- ng_overlap %>% 
  mutate(db = coalesce(db, nat_geo))

total_ng_overlap %>% 
  tabyl(db) %>% 
  adorn_totals()

sf_ng_overlap <- total_ng_overlap %>% 
  st_as_sf() %>% 
  st_transform(crs = 4326)

tabyl_ng <- sf_ng_overlap %>% 
  tabyl(db) %>% 
  adorn_totals() %>% 
  adorn_pct_formatting()

#save total sites - lat longs files
write_csv(total_ng_overlap, here("data", "total_cc.csv"))

tabyl_ng %>% 
  write.csv(here("data", "percent_cc.csv"))
